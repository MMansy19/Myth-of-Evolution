...
function PostAIDialog({ post, onClose }: { post: { id: string; title: string; content: string }; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 flex items-start justify-center pt-4 sm:pt-6 p-3 bg-black/60 backdrop-blur-2xl animate-slide-from-top"
         style={{ zIndex: 2147483600 }} onClick={onClose}>
      <div className="w-full max-w-2xl rounded-3xl flex flex-col max-h-[92vh] overflow-hidden border border-white/15 glass-strong"
